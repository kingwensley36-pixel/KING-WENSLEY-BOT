    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                ...
                const messageText = `
╔══════════════════╗
      *𝙺𝙸𝙽𝙶 𝚆𝙴𝙽𝚂𝙻𝙴𝚈 𝙰𝙸👑 Bot Connected Successfully* 🚀
╠══════════════════╣
...
*Digital Crew 243*
                ...
                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💻 Powered by KING WENSLEY',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 50947831480; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '50947831480' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'WENSKING);
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['50947831480@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
